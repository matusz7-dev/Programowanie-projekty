...
bool isGoodType(string element)
{
    ...
    if (element[0] <= '0' || element[0] > '9') 
        return false;
    //Zeby nie było 05
    for (size_t position = 1; position < element.length(); ++position)
    {
        if (element[position] > '9' || element[position] < '0')
            return false;
    }
    return true;
}
...
int startGame()
{
    ...
    {
        int boardSize=stoi(boardSizeString);
        if (boardSize <= 0)
        {
...
void updateBoard(vector<vector<string>>& board)
{
    int boardSize = board.size();
...
void runTicTacToe(vector<vector<string>>board)
{
    int boardSize = board.size();
    displayBoard(board);
...
int main()
{
    ...
    vector<vector<string>>vectorBoard = createBoard(boardSize);
    runTicTacToe(vectorBoard);
}